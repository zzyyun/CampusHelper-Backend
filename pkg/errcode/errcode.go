...
const (
	...
	// ErrInvalidWechatCode 同 10001，便于前端按参数错误识别（alias）
	ErrInvalidWechatCode = 10001
...
// 映射规则：
//   - 0            → 200
//   - 1xxxx        → 502（第三方依赖错误对外表现为网关错误）
//   - 2xxxx        → 401 / 403（按具体子段）
//   - 3xxxx        → 429
//   - 4xxxx        → 400
//   - 5xxxx        → 502（下游不可达）/ 504（超时）/ 500
//   - 9xxxx        → 500
//   - 未知段位     → 500
func HTTPStatus(code int) int {
	...
	case code >= 20000 && code < 30000:
		// 鉴权段细分：20006/20007 是 403，其他是 401
...
// 映射规则（gRPC Code → 业务码）：
//   - OK                  → (Success, 200)
//   - Canceled            → (ErrInternal, 500)
//   - Unknown             → (ErrInternal, 500)
//   - InvalidArgument     → (ErrDownstreamInvalidArgument, 400)
//   - DeadlineExceeded    → (ErrDownstreamTimeout, 504)
//   - NotFound            → (ErrDownstreamNotFound, 404)
//   - AlreadyExists       → (ErrInvalidParam, 400)
//   - PermissionDenied    → (ErrInsufficientPermission, 403)
//   - ResourceExhausted   → (ErrRateLimited, 429)
//   - FailedPrecondition  → (ErrInvalidParam, 400)
//   - Aborted             → (ErrInternal, 500)
//   - OutOfRange          → (ErrInvalidParam, 400)
//   - Unimplemented       → (ErrDownstreamInternal, 500)
//   - Internal            → (ErrDownstreamInternal, 500)
//   - Unavailable         → (ErrDownstreamUnavailable, 502)
//   - DataLoss            → (ErrDownstreamInternal, 500)
//   - Unauthenticated     → (ErrInvalidToken, 401)
