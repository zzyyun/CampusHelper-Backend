...
Page({
  ...
  onToggleLike(){if(!api.requireLogin())return;(this.data.isLiked?api.unlikePost:api.likePost)(this.data.postId).then(d=>this.setData({isLiked:d.liked,'post.likes_count':d.likes_count})).catch(()=>wx.showToast({title:'操作失败',icon:'none'}))},
  ...
  onSubmitComment(){if(!api.requireLogin())return;const c=this.data.commentText.trim();if(!c)return wx.showToast({title:'请输入评论内容',icon:'none'})
